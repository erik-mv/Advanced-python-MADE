...
def test_process_queries_can_process_all_queries_from_correct_file(capsys, caplog):
    ...
    with open("quries_utf8.txt") as query_fin:
        ...
        assert any("load inverted index" in message for message in caplog.messages), (
            "there is no 'load inverted index' massage in logs"
        )
        #assert any("two words" in message for message in caplog.messages), (
        #    "there is no 'two words' massage in logs"
        #)
        assert "load inverted index" not in captured.out
        ...
        assert all(record.levelno <=logging.WARNING for record in caplog.records), (
            "application isunstable, there are WARNING+ level messages in logs"
        )
