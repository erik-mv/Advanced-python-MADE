...
def callback_build(arguments):
    ...
    *arguments - load arguments
    """
    logger.debug(
        "call build subcommand with arguments: %s and %s", 
        arguments.dataset, arguments.output,
    )
...
def setup_logging():
    ...
    #    logging.config.dictConfig(yamlconfig_fin.read())

    simple_formtter = logging.Formatter (
    ...
    file_handler.setFormatter(simple_formtter)
